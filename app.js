...
let cols = 48, rows = 30;
let cells = makeGrid(), ages = makeGrid(), trails = makeTrailGrid();
...
function makeTrailGrid() { return Array.from({length: rows}, () => new Float32Array(cols)); }
...
function clear(){setRunning(false);cells=makeGrid();ages=makeGrid();trails=makeTrailGrid();effects=[];generation=0;history=[0];selectPreset('blank');draw();drawChart()}
function randomize(){setRunning(false);cells=makeGrid();ages=makeGrid();trails=makeTrailGrid();for(let y=0;y<rows;y++)for(let x=0;x<cols;x++)if(Math.random()<.27){cells[y][x]=1;ages[y][x]=1}generation=0;history=[population()];selectPreset('random');draw();drawChart()}
...
function loadPattern(name){setRunning(false);cells=makeGrid();ages=makeGrid();trails=makeTrailGrid();generation=0;history=[0];if(name==='glider')place(patterns.glider,Math.floor(cols/2)-1,Math.floor(rows/2)-1);if(name==='gun')place(patterns.gun,2,Math.floor(rows/2)-4);if(name==='random'){randomize();return}selectPreset(name);history=[population()];draw();drawChart()}
